...
def b_referencia_catastral(html: str) -> dict:
    if found := regex_engine.search("catastral", html, exact_match=False):
        ...
        # Si la referencia catastral forma parte de un link, del estilo:
        # <a href="consultaDnprc.php?rc=9496547VK3799E0010&idSub=SUB-JA-2023-222104" target="_blank" title="Abre datos catastrales en nueva ventana" onclick="return confirm('El Portal de Subastas se va conectar a los servicios web de la Dirección General del Catastro y mostrará la información en una nueva ventana');">9496547VK3799E0010</a>
        # Entonces hay que extraerla del link
        if "consultaDnprc.php?rc=" in found:
            try:
                rc = found.split("rc=")[-1].split("&")[0]
            except IndexError:
                rc = None
        ...
        # Si la referencia catastral no forma parte de un link, del estilo:
        # 9496547VK3799E0010
        # Entonces hay que extraerla del texto
        elif ref := REGEX_REFERENCIA_CATASTRAL.search(found):
            return {
